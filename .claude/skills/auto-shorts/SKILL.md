...
- 사용자가 주제를 줬으면 그대로 간다. 막연하면("재미있는 거 아무거나") **`references/script-writing.md` 의
  카테고리에서 구체 후보 3개를 훅 문장까지 붙여 제안**하고 AskUserQuestion 으로 하나 고르게 한다.
  (질문은 이 한 번뿐. 답이 없거나 자율 실행 상황이면 가장 반전이 큰 후보로 바로 진행.)
...
`bgm` 에 `"file": "경로"` 를 주면 직접 고른 음악을 쓴다.

**대본 체크리스트 (쓰고 나서 반드시 스스로 검사)**
- 첫 씬 첫 문장이 질문·반전·숫자 중 하나로 시작하는 훅인가? 제목과 같은 궁금증인가?
- 총 글자 수 220~300자(공백 포함)면 대략 45~55초. 300자를 넘기면 60초를 넘길 수 있다.
...
- 보통 1~3분 걸린다(AI 이미지 생성이 대부분). 단계별 산출물은 `shorts_output/<slug>/work/` 에 캐시되어
...
1. `shorts_output/<slug>/preview.jpg` 를 **Read 로 열어** 12장 프레임을 본다: 자막이 잘리거나 UI 안전영역
   (하단 20%·우측 15%)에 걸치는지, 이미지가 주제와 맞는지, 헤드라인이 캡션과 겹치는지.
2. `meta.md` 의 길이가 60초 이내인지, 출처 표기가 필요한 자료(CC)가 있는지 본다.
3. 문제가 있으면 `project.json` 만 고치고 다시 렌더한다:
   - 이미지가 엉뚱함 → 그 씬의 `image_prompt`/`keywords` 를 더 구체적으로. `card` 로 떨어진 씬은
     프롬프트를 짧고 안전하게 바꾸거나 `--force` 로 한 번 더 시도.
...
- **품질**: AI 이미지는 씬마다 다른 구도, 한 영상 안에서는 `image_style` 로 톤 통일. 켄 번즈 방향이 씬마다
  ...
  BGM 은 나레이션에 맞춰 자동 덕킹, 최종 -14 LUFS(유튜브 기준).
